...
components: ["RippleDemo", "RippleDemoWithoutFadeOut"]
---

import RippleDemo from "./RippleDemo";
import RippleDemoWithoutFadeOut from "./RippleDemoWithoutFadeOut";
...
```js
const fadeOutRipple = (ripple: HTMLSpanElement, animationStart) => {
...
Now, at the end of the `createRipple(e)` function, we add the `fadeOutRipple()` function to the following event listeners:
